...
def insert_data_to_hana(df: pd.DataFrame):
    ...
    # Prepare the INSERT statement with mappings
    insert_statement = """
    ...
    try:
        if data_to_insert:
            ...
            print("No records found. Inserting two sample records...")
            sample_data = [
                (5000000001, "K3", "SRC", 106, 208, 307, 400, "SUM", 500, "PM", 600, "CK", "AKT", "CK", 700, 800, "RM", "CK", 900, "RA", "CK", "Y", "N", 1000, 1100, 1200, 1300, "Y", "N", "Y", 1400, "N"),
                (5000000002, "K4", "SRC", 107, 209, 303, 401, "SUM", 501, "PM", 601, "CK", "AKT", "CK", 701, 801, "RM", "CK", 901, "RA", "CK", "N", "Y", 1001, 1101, 1201, 1301, "N", "Y", "N", 1401, "Y")
            ]
            cursor.executemany(insert_statement, sample_data)
            conn.commit()
            print("Sample records inserted successfully into HANA Cloud.")
...
def run_ship_to_profile(ekp: int, kalknr: str = None):
    ...
    if not df_res.empty:
        ...
        insert_data_to_hana(df_res)  # This will trigger the insertion of sample records

# Execute with specific EKP and kalknr for demonstration
#run_ship_to_profile(5000959631)
run_ship_to_profile(5003243537)